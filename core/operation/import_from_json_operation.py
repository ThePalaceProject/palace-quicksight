...
class ImportFromJsonOperation(BaseOperation):
    ...
    def __init__(self, template_name: str, target_namespace: str, data_source_arn: str, input_dir: str, *args,
        ...
        super(ImportFromJsonOperation, self).__init__(*args, **kwargs)

    def execute(self):
        # Read template file into dictionary
        template_data = None
        template_file = self._resolve_path(self._intput_dir, TEMPLATE_DIR, self._template_name + ".json")
        ...
        arn, version_arn, template_id = self._create_or_update_template_from_template_definition(
        ...
        for di in dataset_configurations:
            ...
            dataset_filename = self._resolve_path(self._intput_dir, DATA_SET_DIR, placeholder + ".json")
            ...
            dataset["DataSetId"] = self._resolve_data_set_id_from_placeholder(placeholder=placeholder,
                                                                              namespace=self._target_namespace)
            dataset["Name"] = dataset["Name"]
            arn, data_set_id = self._create_or_update_data_set(dataset_definition=dataset)
